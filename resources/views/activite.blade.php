  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="/saveact"   method="post">
        {{csrf_field()}}
        <div class="modal-body">
          <div class="form-group">
            <label for="recipient-name" class="col-form-label">type_activite</label>
            <input type="text" name="type_activite" class="form-control" id="recipient-name">
          </div>
          <div class="form-group">
            <label for="recipient-name" class="col-form-label">nombre_activite</label>
            <input type="text"   name="nb_activite" class="form-control" id="recipient-name">
          </div>
          <div class="form-group">
            <label for="recipient-name" class="col-form-label">nombre_eleve</label>
            <input type="text"   name="nb_eleve" class="form-control" id="recipient-name">
          </div>
...
<div class="modal fade" id="deletemodelpop" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" id="delete_activite">
    ...
    <br> </br>
    <h3 style="text-align:center;margin-left:5%;margin-bottom:-5%;">&ensp; &ensp; &ensp; Liste des activités</h3>
 ...
 <table id="table1" class="table">
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">nombre_activite</th>
    
      <th scope="col">nombre_eleve</th>
...
@foreach ($activite as $data)
    <tr>
      ...
      <td>{{$data->type_activite}}</td>
      <td>{{$data->nb_activite}}</td>
      <td>{{$data->nb_eleve}}</td>
     

      <td><a href="{{ url('act-edit/'.$data->id)}}" class="btn btn-success">EDIT  </a>          </td>
...
$(document).ready( function () {
   ...
   console.log(data);
    $('#delete_activite').val(data[0]);
    $('#delete_model_form').attr('action','/act-delete/'+data[0]);
